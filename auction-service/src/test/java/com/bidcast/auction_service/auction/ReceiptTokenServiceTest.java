...
class ReceiptTokenServiceTest {
    ...
    void generateAndValidate_Success() {
        ...
        String token = tokenService.generateReceiptId(sessionId, bidId, advertiserId, price);
        assertNotNull(token);

        ValidatedReceipt receipt = tokenService.validateAndExtract(token, sessionId, bidId, 600);
        
        assertEquals(advertiserId, receipt.advertiserId());
        assertEquals(0, price.compareTo(receipt.advertiserBidPrice()));
    }
    ...
    void validate_FailsIfPriceIsManipulated() {
        ...
        String token = tokenService.generateReceiptId(sessionId, bidId, "adv-1", originalPrice);
        ...
        assertThrows(InvalidReceiptSignatureException.class, () -> {
            tokenService.validateAndExtract(tamperedToken, sessionId, bidId, 600);
        });
    }
    ...
    void validate_FailsIfSessionMismatch() {
        String token = tokenService.generateReceiptId("sess-A", UUID.randomUUID(), "adv-1", BigDecimal.ONE);

        assertThrows(IllegalArgumentException.class, () -> {
            tokenService.validateAndExtract(token, "sess-B", UUID.randomUUID(), 600);
        });
    }
    ...
    void validate_FailsIfExpired() throws InterruptedException {
        ...
        String token = tokenService.generateReceiptId("sess-1", UUID.randomUUID(), "adv-1", BigDecimal.ONE);
        
        // Esperamos 1 segundo para asegurar que el timestamp sea anterior al 'now'
        Thread.sleep(1100);

        assertThrows(IllegalArgumentException.class, () -> {
            tokenService.validateAndExtract(token, "sess-1", UUID.fromString(token.split(":")[1]), 0);
        });
    }
    ...
    void validate_FailsIfFormatIsInvalid() {
        assertThrows(IllegalArgumentException.class, () -> {
            tokenService.validateAndExtract("formato:invalido:sin:partes", "sess-1", UUID.randomUUID(), 600);
        });
    }
}
